...
class Util {
    public static func getObject<T:Codable>(input: [String:Any]?) -> T? {
    ...
    public static func getObjectArray<T:Codable>(input: [[String:Any]]?) -> T? {
        guard let input = input else {return nil}
        do {
            let data = try JSONSerialization.data(withJSONObject: input)
            let object = try JSONDecoder().decode(T.self, from: data)
    ...
    public static func convertToArrayDict<T:MoEngageDictionaryConvertible>(input:[T]) throws -> [[String: Any]]? {
         ...
         do {
             ...
             arrayDict = try JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [[String: Any]]
